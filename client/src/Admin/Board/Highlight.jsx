...
function HighlightOne() {
  ...
  const handleMainImageChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        setUpdateData({ ...updateData, thumbnail: imageUrl });
  ...
  const handleVideoChange = async (e) => {
    ...
    if (file) {
      try {
        ...
        setUpdateData({ ...updateData, video: VideoUrl });
        setMessage({ text: "Main Video uploaded successfully!", type: "success" });
      } catch (err) {
        setError(err.message);
      } finally {
        setIsLoading(false);
  ...
  const uploadVideo = async (file) => {
    ...
    imageData.append('upload_preset', UPLOAD_PRESET);
    try {
      const response = await fetch(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/video/upload`, { method: 'POST', body: imageData });
